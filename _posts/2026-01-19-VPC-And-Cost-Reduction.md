...
# [AWS] VPC 실습 비용 아끼기 (feat. CloudNuke)
  <br><br>
  
## 1. 꼬여버린 코드와 불어나는 요금

요즘 AWS에 관심이 많아 한창 관련된 실습을 많이 하고 있었습니다. 며칠에 걸쳐 다양한 시도를 하다 보니 어느새 AWS 리소스는 중구난방으로 생성되기 시작했습니다.

무엇보다 가장 큰 문제는 **'비용'**이었습니다. 실습을 위해 띄워둔 리소스들이 요금을 상당량 발생시키고 있었습니다.

![AWS Cost Explorer - EC2-Other 비용 그래프](/images/aws-cost.png)

그래프에서 **보라색 영역이 EC2-Other 서비스 요금**입니다. 이 EC2-Other에는 NAT Gateway와 Elastic IP(EIP) 요금이 포함되어 있습니다. 결론부터 말하자면, Terraform 코드에서 NAT와 EIP 리소스를 주석 처리하여 이 비용을 절감할 수 있었습니다.
